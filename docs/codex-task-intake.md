...
- `project`：任务属于哪个产品、领域或长期事项，例如 `Time-butler`、`投资Agent`、`个人事务`。
...
App 的本地任务解析器会把明确的面试回答任务归为 `写作输出`，包括包含“面试题 / 面经 / 逐字稿 / 回答”的任务，以及以“优化问题：”或“优化题目：”开头的题目打磨任务。规则只在用户没有手动选择分类时生效；显式分类始终优先。
...
当 Codex 不通过 UI 表单，而是直接写入 `../time-butler-data/Time-butler.db` 时，必须在最终回复前完成：
