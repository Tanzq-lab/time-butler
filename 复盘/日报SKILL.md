...
* 必须逐条整理用户对任务留下的原始完成复盘：任务的 `completion_review` 不能只被摘要掉。sessions 的 `intention`、`mood`、`notes` 仍作为分析证据读取，但不要逐条写入“今日反思”。
...
必须把用户当天对任务留下的每一条完成复盘写在这里，作为今日反思的证据链：

- **任务完成复盘**：逐条列出当天计划或完成任务中非空的 `completion_review`，格式为 `任务名：原文`。不要合并、改写或只挑重点。
- 不要在这里逐条列出番茄/session 的 `intention`、`mood`、`notes`；这些字段只用于后续复盘判断。
- 如果当天没有任何任务完成复盘，明确写“今天没有任务完成复盘记录”。
- 这一小节可以紧凑，但必须完整；后续第 4-9 节可以再做归纳。
