...
你的主任务仍然是日报复盘。日报写入并验证完成后，还要按本文件的「复盘后产品优化建议与对话确认规则」完成产品审计，把有证据的建议直接写进本次 Codex 自动任务的最终回复，等待用户在对话中选择；定时日报不得自行实施产品优化。
...
产品路径分析和产品优化建议不写入 `time_pages`，也不另存为长期报告文件；它们只作为本次运行的临时分析证据，并在 Codex 对话的最终回复中交付。
...
“提交 Git”“项目提交 Git”“Git 收尾”等表述，只授权处理 **进入本步骤时已经存在** 的修改：允许只读检查、理解意图、分组、验证、暂存和创建本地 commit；不授权产生新的文件内容变化。

* 禁止为了完成提交而修改、补写、格式化、重命名或删除文件，也禁止修复 TODO、文章空白、元数据、测试或校验错误。尤其不得根据问题标题、上下文或推测替用户补写尚未回答的内容；用户没写的内容必须保持原样。
* 已经存在的修改、删除或重命名可以按原样分组提交；这里禁止的是 Git 收尾步骤自行制造新的修改。
* 验证失败、内容未完成或意图不清时，只能跳过该组并按下文要求报告和提问，不能先修改文件再提交。
* 只有用户在 Git 提交授权之外，另行明确提出具体内容修改时，才可以编辑相应文件；“把项目提交 Git”本身不构成内容修改授权。
* 在任何写操作前记录工作区与暂存区基线；提交前复核暂存 diff，确认每个暂存变更都来自该基线。Git 收尾期间若出现基线之外的内容变化，停止该组，不得把新变化提交进去。
...
Git 收尾的默认结果是“把所有可以安全解释、隔离和验证的现有修改提交掉”，不是只检查后把工作区原样留给用户。一个修改组失败或存在疑问时，继续处理其他独立修改组，不得把整个仓库一并放弃。
...
* 优先运行只覆盖本组明确文件或功能的验证，并对暂存内容运行 `git diff --cached --check`。
* 如果仓库规则强制要求全仓验证，先记录验证基线。全仓验证若因本组之外、运行前已经存在的问题失败，不得谎称本组通过，也不得用模糊的“验证失败”结束：必须在最终回复中列出失败检查、具体无关文件、为什么它阻止提交，并提出一个用户可直接回答的处理问题。
* 全仓基线问题只阻塞确实受该强制门禁约束的修改组；不受该门禁影响且能独立验证的其他组仍应继续提交。

以下情况必须跳过，不得为了“全部提交”牺牲安全：

* 正在 merge、rebase、cherry-pick、revert，存在未解决冲突或 detached HEAD。
* 凭据、密钥、令牌、数据库备份、运行日志、缓存、构建产物、依赖目录或其他不应入库的机器运行数据。
* 仓库说明明确禁止提交的私密数据；但 `../time-butler-data` 是用户许可的私有数据仓库。完成写入和 SQLite 验证后，允许且应当单独提交范围内的 `Time-butler.db`，以及本次允许的数据操作确实改变的 `data/pomodoro-estimation-log.jsonl`。绝不提交 `backups/`、`logs/`、`*.db-wal`、`*.db-shm`、密钥或无关数据。
* 已有修改相互交织，无法通过路径级暂存安全拆分。
* 需要 amend、rebase、reset、checkout、clean、强制操作或改写历史才能完成。

Git 收尾只创建本地 commit，不执行 `git push`，不创建 PR，不改远端。不能为了让现有修改通过验证而顺手扩展功能；如果需要额外代码修改，留待用户在 Codex 对话中确认后作为独立任务处理。

每个跳过项必须在最终 Codex 对话中写清：

* 仓库和明确文件路径。
* 已经确认的修改意图。
* 阻塞原因或验证失败。
* 需要用户回答的一个具体问题；如果不需要用户判断，说明下一次自动任务怎样才能继续。
...
在判断任何产品 / 工具优化之前，必须先读取 `/Users/amos/time-butler/docs/product-optimization-methodology.md`，并按其中要求读取 `/Users/amos/AmosTan` 的产品方法论索引和相关原始方法笔记。

定时日报的产品审计采用“建议进入对话、用户确认后再实施”的闭环。即使建议预计不超过 4 个番茄、可以安全验证，本次定时运行也只提出建议，不修改 Time Butler 源码、脚本、SKILL、配置或测试，不创建产品优化 commit。用户在 Codex 对话中选择或调整建议后，再作为独立请求按 `AGENTS.md` 的产品优化方法实施、验证和提交。

### 1. 必须主动扫描的建议来源

在判断“没有值得提出的优化建议”之前，必须完成这些动作：
...
- 是否值得进入对话：是 / 否
- 原因：证据充分且存在最小变化 / 单日观察 / 涉及历史数据修改 / 超过合理范围 / 证据不足
```

### 2. 对话交付格式

最终 Codex 回复最多列 3 条证据充分的产品优化建议。每条必须包含：

```md
- 用户路径：用户实际经过的页面、操作或自动化流程。
- 提炼需求：用户真正要完成什么，不从功能点开始。
- 关键假设：什么条件成立时，这个变化才会有帮助。
- 最小变化：等待用户确认后可以实施的最小改动。
- 验证方式：如何用本地测试、事件或可复现路径判断有效。
- 风险边界：不应触碰的数据、状态或工作流。
- 建议状态：待用户选择。
```

只有单日小样本、埋点覆盖不足或缺少交叉证据时，明确标为“观察”，不能伪装成建议。没有证据充分的建议时，直接写“本次没有需要你决策的产品优化建议”，不为填满数量制造问题。

产品建议和工程审计不得写入个人日报 AI 区块，也不得另建产品建议 Markdown / JSON 作为用户交付物。临时路径分析文件只供本次运行读取，任务结束后删除。Codex 对话中的最终回复才是产品审计的交付面。

如果用户指出“你没有检查 App / 没有根据记录提出优化 / 没有把结果送回对话”，必须先更新 `docs/codex-mistake-notebook.md` 和本 SKILL 中的相关规则，再继续实现功能。
...
- 产品优化建议：无 / X 条待用户选择（逐条按“用户路径、提炼需求、关键假设、最小变化、验证方式、风险边界”列出）
...
* 不要把没有数据支撑的推断写成事实。
* 不要泄露与当天复盘无关的隐私细节。
* 定时日报不得自行实施产品优化；必须先在 Codex 对话中提出建议并等待用户确认。
