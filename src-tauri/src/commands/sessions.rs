...
async fn reassign_completed_pomo_in_pool(
    ...
    .ok_or_else(|| "只能更正已完成且已计入任务的专注番茄。".to_string())?;

    let source_task_id = record
        .try_get::<i64, _>("source_task_id")
...
mod tests {
    ...
    async fn setup_pool(include_activity_log: bool) -> SqlitePool {
        ...
        .unwrap();
        sqlx::query(
            "INSERT INTO sessions (id, task_id, phase, completed, pomo_counted, category_id) VALUES (93, 12, 'work', 1, 1, 48)",
        )
        .execute(&pool)
        .await
        .unwrap();
