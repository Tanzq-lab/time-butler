...
- 使用 npm 安装缺失依赖。
- 检查 Rust / cargo 是否可用。
- 停掉旧打包 App、孤立 dev runtime 和 1420 端口上残留的旧本地服务。
- 注销并清理 `src-tauri/target/**/bundle/` 中可被 LaunchServices 误启动的生成 App。
- 启动 Tauri 开发热更新版。
...
- 日常与 Codex 启动入口：只使用 `open /Users/amos/time-butler/open-time-butler-dev.command`；`npm run tauri dev` 只是启动器内部实现，不作为另一个入口。
...
- `docs/ui-optimization-playbook.md`：全站 UI 改造复盘、当前视觉合同、响应式/可访问性门禁和品牌图标规则。
...
docs/                         Codex 任务录入、产品优化、UI 和番茄预估文档
