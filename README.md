...
把当前 Tauri 运行数据库和新产生的估算日志同步到私密数据仓库：

```zsh
npm run data:backup
```

这个命令不会改变 App 的运行数据库路径。App 仍然只使用一个真实数据库：`sqlite:Kairos-Pomodoro.db`。备份时使用 SQLite `.backup`，可以在 App 开着时取得一致快照；如果本仓库临时产生了 `data/pomodoro-estimation-log.jsonl`，脚本会去重合并到私密仓库后删除本仓库里的临时日志。
...
- 添加任务后，追加记录到 `data/pomodoro-estimation-log.jsonl`；分享代码前用 `npm run data:backup` 合并到私密数据仓库。
...
data/                         运行时临时 JSONL 日志目录，内容会被 git 忽略
scripts/private-data.mjs      同级私密数据仓库初始化和备份脚本
